...
## 페이지 섹션 순서
Hero → 앨범커버 → Listen Now(플레이어) → 가사 → Discography → Artist(바이오/손글씨 노트/근황/구독) → Footer(스트리밍 링크)

## 콘셉트 설정 (AI 스터디 과제용 가상 설정)
- "소하"는 가상의 인디 싱어송라이터. **AI 생성 곡이라는 사실은 페이지에 드러내지 않는다**
- 가상 설정으로 남아있는 것: 레이블 "달선 레코드"(푸터), 디스코그래피 3장(늦은 배웅 / 창문 너머 / 자라나), 바이오 속 지명·이력
- 릴리즈 크레딧, 라이브 일정, 프레스 인용은 사용자 요청으로 제거함 (다시 넣지 말 것)
